...
class CapturingMockCompleter(TokenCompleter):
    ...
    async def __call__(self, model_input: tinker.ModelInput, stop: list) -> TokensWithLogprobs:
...
async def test_rl_integration(adapter, expected_messages, expected_tokenized_text):
    ...
    assert len(capturing_completer.captured_observations) == 1, "Expected exactly 1 completer call"
