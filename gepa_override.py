# This overrides dspy/teleprompt/gepa/gepa.py, to add the reflection_prompt_template parameter.
import inspect
import logging
import random
import sys
...
# Add the local gepa path to sys.path to use the local version
_gepa_local_paths = [
    "/nas/ucb/biddulph/gepa-legibility/gepa/src/",
    "/nas/ucb/biddulph/gepa-legibility/",
]
for _gepa_local_path in _gepa_local_paths:
    if _gepa_local_path not in sys.path:
        sys.path.insert(0, _gepa_local_path)

# Force remove any cached gepa module to ensure we get the local one
for key in list(sys.modules.keys()):
    if key.startswith("gepa") and "dspy" not in key:
        del sys.modules[key]
...
class GEPA(Teleprompter):
    ...
    def compile(
        ...
        *,
        ...
        # Ensure local gepa is used
        for _gepa_local_path in _gepa_local_paths:
            if _gepa_local_path not in sys.path:
                sys.path.insert(0, _gepa_local_path)

        # Force remove any cached gepa module to ensure fresh import
        for key in list(sys.modules.keys()):
            if key.startswith("gepa") and "dspy" not in key:
                del sys.modules[key]
